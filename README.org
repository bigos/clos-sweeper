...
Regular Windows SBCL does not support image compression. Possibly I could build
SBCL from source to support it.
...
I have a collection of dlls that work and now I have to trim the rest. But naiive
trimming does not work. Possibly the only way to distribute the Windows version
is to use MSYS2.

** generating again
remove src/clos-sweeper.exe
run ./builder.ros
run ./prepare_distribution.sh
