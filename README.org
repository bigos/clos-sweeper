...
I added build_distribution.sh that is meant to be run from the MSYS2 terminal
  cd /c/Users/jacek/Programming/Lisp/clos-sweeper/
  ./build_distribution.sh


But I had no luck. Copying all dlls and trying to run the app gives me error:
  The alien function "g_error_free" is undefined.

It is somewhat related to incorrect GTK enviroment

For some reason trying to run deploy did not give me an app that would run successfully.
